...
public class ChessBoardTest extends TestCase {
	...
	public void testHas_MaxBoardWidth_of_7() {
		assertEquals(7, ChessBoardConstants.MAX_BOARD_HEIGHT);
	}
	...
	public void testIsLegalBoardPosition_False_X_equals_11_Y_equals_5() {
		...
		// assertTrue(isValidPosition); This test case seems to be false so I have
		// modified it
	...
	public void testLimits_The_Number_Of_Pawns() {
		for (int i = 0; i < 10; i++) {
			...
			int row = i / ChessBoardConstants.MAX_BOARD_WIDTH;
			testSubject.addPiece(pawn, 6 + row, i % ChessBoardConstants.MAX_BOARD_WIDTH, PieceColor.BLACK);
			if (row < 1) {
				assertEquals(6 + row, pawn.getXCoordinate());
				assertEquals(i % ChessBoardConstants.MAX_BOARD_WIDTH, pawn.getYCoordinate());
			} else {
